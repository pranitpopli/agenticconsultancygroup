...
const ScenarioModelling = ({ scenarios }: Props) => {
        ...
        {scenarios.map((scenario) => {
          ...
          return (
            <motion.button
              ...
              className={`text-left border p-5 transition-all duration-200 relative group ${
                isSelected
            ...
            >
              ...
              {/* Expand indicator */}
              <div className={`mt-4 pt-3 border-t border-border transition-colors ${isSelected ? "" : "opacity-0 group-hover:opacity-100"}`}>
                <span className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">
                  {isSelected ? "Hide details" : "View trade-offs"}
                </span>
              </div>
            </motion.button>
          );
      ...
      <div className="mt-8 overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0">
        <div className="border border-border overflow-hidden min-w-[480px]">
          <div className={`grid grid-cols-[1.5fr_repeat(${scenarios.length},1fr)] border-b border-border`}>
            ...
            {scenarios.map((s) => (
              <div key={s.id} className={`p-3 text-[10px] uppercase tracking-[0.12em] border-l border-border ${s.recommended ? "text-foreground font-medium" : "text-muted-foreground"}`}>
                {s.name}
                {s.recommended && " ★"}
              </div>
            ))}
          ...
          {[
            ...
            <div key={row.label} className={`grid grid-cols-[1.5fr_repeat(${scenarios.length},1fr)] ${i > 0 ? "border-t border-border" : ""}`}>
              ...
              {scenarios.map((s) => (
                <div key={s.id} className={`p-3 text-xs border-l border-border capitalize ${s.recommended ? "text-foreground" : "text-muted-foreground"}`}>
                  {row.render(s)}
                </div>
              ))}
            </div>
