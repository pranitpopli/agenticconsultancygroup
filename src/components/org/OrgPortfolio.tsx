...
const OrgPortfolio = () => {
  ...
  return (
    <div>
      {/* Summary cards */}
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-14">
        {[
            ...
            <div className="flex-1 flex">
              {Array.from({ length: Math.ceil(maxWeek / 4) }, (_, i) => (
                <div key={i} className="flex-1 p-2 text-center border-r border-border last:border-r-0">
                  <span className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">Month {i + 1}</span>
                </div>
              ))}
            </div>
          </div>
          {PORTFOLIO_PROJECTS.map((proj, pi) => (
            <div key={proj.id} className={`flex items-center min-w-[800px] ${pi > 0 ? "border-t border-border" : ""}`}>
              <div className="w-[200px] flex-shrink-0 p-3 border-r border-border">
                <p className="text-xs text-foreground font-medium truncate">{proj.name}</p>
                <div className="flex items-center gap-2 mt-1">
                  <span className={`text-[9px] uppercase tracking-[0.1em] px-1.5 py-0.5 rounded-sm ${STATUS_STYLES[proj.status]}`}>{proj.status.replace("-", " ")}</span>
              ...
              <div className="flex-1 relative h-14">
                <div className="absolute inset-0 flex">
                  {Array.from({ length: maxWeek }, (_, i) => (
                    <div key={i} className={`flex-1 ${(i + 1) % 4 === 0 ? "border-r border-border/50" : ""}`} />
                  ))}
                </div>
                <motion.div
                  ...
                  className={`absolute top-2.5 bottom-2.5 rounded-sm flex items-center px-2 ${
                    proj.status === "blocked" ? "bg-[hsl(var(--status-danger)/0.15)]" :
                    proj.status === "at-risk" ? "bg-[hsl(var(--status-warning)/0.15)]" : "bg-foreground/10"
                  }`}
                  style={{
                ...
                >
                  <div
                    className={`absolute left-0 top-0 bottom-0 rounded-sm ${
                      proj.status === "blocked" ? "bg-[hsl(var(--status-danger)/0.25)]" :
                      proj.status === "at-risk" ? "bg-[hsl(var(--status-warning)/0.25)]" : "bg-foreground/15"
                    }`}
                    style={{ width: `${proj.progress}%` }}
                  />
                  <span className="text-[9px] text-foreground/60 relative z-10 truncate">W{proj.startWeek}–W{proj.endWeek}</span>
                </motion.div>
              </div>
            </div>
          ))}
      ...
      <section className="mb-14">
        ...
        <div className="space-y-3">
          {SWARM_ALERTS.map((alert) => (
            <AlertCard key={alert.id} alert={alert} expanded={expandedAlert === alert.id} onToggle={() => setExpandedAlert(expandedAlert === alert.id ? null : alert.id)} />
          ))}
        </div>
      </section>
              ...
              <div className="p-3 border-l border-border">
                <div className="flex flex-wrap gap-1.5">
                  {conflict.projects.map((proj) => (
                    <span key={proj.projectId} className="text-[10px] px-2 py-0.5 border border-border text-foreground/80 rounded-sm">{proj.projectName}</span>
                  ))}
                </div>
              </div>
              <div className="p-3 border-l border-border flex items-center">
                <span className={`w-1.5 h-1.5 rounded-full mr-2 ${conflict.severity === "critical" ? SEVERITY_DOT.critical : SEVERITY_DOT.warning}`} />
                <span className="text-[10px] uppercase tracking-[0.1em] text-muted-foreground">{conflict.severity}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="border border-border">
          <div className="p-5 border-b border-border">
            <div className="flex items-baseline justify-between mb-3">
              <p className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground">Total: £{(PORTFOLIO_TOTALS.totalBudget / 1000).toFixed(0)}k committed</p>
              <p className="text-sm text-foreground tabular-nums font-sans">£{(PORTFOLIO_TOTALS.totalSpent / 1000).toFixed(0)}k spent ({Math.round(PORTFOLIO_TOTALS.totalSpent / PORTFOLIO_TOTALS.totalBudget * 100)}%)</p>
            </div>
            <div className="h-2 bg-secondary rounded-full overflow-hidden">
              <motion.div initial={{ width: 0 }} animate={{ width: `${(PORTFOLIO_TOTALS.totalSpent / PORTFOLIO_TOTALS.totalBudget) * 100}%` }} transition={{ duration: 1 }} className="h-full bg-foreground/30 rounded-full" />
            </div>
          </div>
            ...
            {PORTFOLIO_PROJECTS.map((proj) => {
              ...
              return (
                <div key={proj.id} className="p-4 flex items-center gap-4">
                  <div className="w-[180px] flex-shrink-0"><p className="text-xs text-foreground font-medium truncate">{proj.name}</p></div>
                  <div className="flex-1">
                    <div className="h-1.5 bg-secondary rounded-full overflow-hidden">
                      <div className={`h-full rounded-full transition-all ${overBurn ? "bg-[hsl(var(--status-danger))]" : "bg-foreground/25"}`} style={{ width: `${pct}%` }} />
                    </div>
                  </div>
                  <div className="w-[120px] flex-shrink-0 text-right">
                    <span className="text-xs tabular-nums text-foreground/80">£{(proj.spent / 1000).toFixed(0)}k / £{(proj.budget / 1000).toFixed(0)}k</span>
                  </div>
                </div>
              );
            })}
...
function AlertCard({ alert, expanded, onToggle }: { alert: SwarmAlert; expanded: boolean; onToggle: () => void }) {
  return (
    <button onClick={onToggle} className={`w-full text-left border border-border border-l-[3px] p-4 transition-colors hover:bg-muted/30 ${SEVERITY_STYLES[alert.severity]}`}>
