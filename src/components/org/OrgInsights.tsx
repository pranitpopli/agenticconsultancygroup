...
const OrgInsights = () => {
  ...
  const filtered = filterCategory === "all" ? SWARM_INSIGHTS : SWARM_INSIGHTS.filter((i) => i.category === filterCategory);
      ...
      <div className="space-y-4">
        <AnimatePresence>
          {filtered.map((insight, i) => (
            <motion.div key={insight.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ delay: i * 0.05 }}>
              <InsightCard insight={insight} expanded={expandedId === insight.id} onToggle={() => setExpandedId(expandedId === insight.id ? null : insight.id)} />
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
...
function InsightCard({ insight, expanded, onToggle }: { insight: SwarmInsight; expanded: boolean; onToggle: () => void }) {
  ...
  return (
    <button onClick={onToggle} className="w-full text-left border border-border p-5 transition-colors hover:bg-muted/20">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0 flex-1">
          <Icon className="w-4 h-4 text-muted-foreground mt-0.5 flex-shrink-0" strokeWidth={1.5} />
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <span className="text-[9px] uppercase tracking-[0.12em] px-1.5 py-0.5 border border-border text-muted-foreground">{meta.label}</span>
                  ...
                  <ul className="space-y-1">
                    {insight.evidence.map((e, i) => (
                      <li key={i} className="text-xs text-foreground/70 flex items-start gap-2">
                        <span className="text-muted-foreground/40 mt-0.5">·</span>{e}
                      </li>
                    ))}
                  </ul>
                </div>
