...
const FacetGroup = ({ label, options, selected, onToggle, counts }: FacetGroupProps) => {
  ...
  return (
    <div className="space-y-2.5">
      <span className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground/70 font-semibold block">{label}</span>
      {showSearch && (
        <div className="relative">
          <Search className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-muted-foreground/40 pointer-events-none" strokeWidth={1.5} />
          <input
            value={facetSearch}
            onChange={(e) => setFacetSearch(e.target.value)}
            placeholder={`Filter…`}
            className="w-full text-[11px] bg-muted/30 border border-border/50 rounded-md pl-7 pr-2.5 py-1.5 outline-none focus:border-foreground/20 focus:bg-background transition-colors placeholder:text-muted-foreground/40 text-foreground"
          />
        </div>
      )}
      <div className={`space-y-px ${showSearch ? "max-h-40 overflow-y-auto pr-1 scrollbar-thin" : ""}`}>
        {filtered.map((opt) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center justify-between px-2 py-[5px] text-[11px] transition-colors rounded-md group ${
                isSelected
            ...
            >
              <span className="flex items-center gap-2 min-w-0">
                <span className={`w-3.5 h-3.5 rounded-[3px] border flex-shrink-0 flex items-center justify-center transition-all ${
                  isSelected ? "border-foreground bg-foreground" : "border-border/70 group-hover:border-foreground/30"
                }`}>
                  {isSelected && <Check className="w-2.5 h-2.5 text-primary-foreground" strokeWidth={2.5} />}
                </span>
                <span className="truncate">{opt}</span>
              </span>
              <span className={`text-[10px] tabular-nums flex-shrink-0 ml-2 ${isSelected ? "text-muted-foreground/70" : "text-muted-foreground/40"}`}>{count}</span>
            </button>
          );
        })}
        {filtered.length === 0 && (
          <p className="text-[11px] text-muted-foreground/40 px-2 py-2">No matches</p>
        )}
      </div>
    </div>
  );
